...
public partial class DbTableGrid<TItem> : ComponentBase where TItem : class
{
    ...
    /// <summary>
    /// Server data callback for MudDataGrid.
    /// </summary>
    private async Task<GridResult<TItem>> LoadServerDataAsync(GridState state)
    {
        // Update query from grid state
        CurrentQuery.PageIndex = state.Page;
        CurrentQuery.PageSize = state.PageSize;

        // Update sorts from grid state
        CurrentQuery.Sorts = state.SortDefinitions
            .Select(s => new SortDef(s.SortBy, s.Descending))
            .ToList();

        // Update filters from grid state (if supported by MudDataGrid)
        // Note: MudDataGrid doesn't directly expose FilterDefinitions in the state,
        // so we maintain filters separately through the filter UI.

        await LoadData();
        await PersistState();

        return CurrentResult ?? new GridResult<TItem>();
    }
    ...
    /// <summary>
    /// Handles row click for opening row details.
    /// </summary>
    private void OnRowClick(DataGridRowClickEventArgs<TItem> args)
    {
        // Row click can expand the detail row; handled by MudDataGrid's DetailTemplate
    }
    ...
    /// </summary>
    private Expression<Func<TItem, string>> GetPropertyExpression(string propertyName)
    {
        var parameter = Expression.Parameter(typeof(TItem), "x");
        var property = Expression.Property(parameter, propertyName);
        var lambda = Expression.Lambda<Func<TItem, string>>(Expression.Convert(property, typeof(string)), parameter);
        return lambda;
    }
