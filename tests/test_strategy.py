...
def create_price_series(prices: list, symbol: str = "AAPL") -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        start_time + timedelta(minutes=i) for i in range(len(prices))
    ]))
...
async def test_strategy_bullish_crossover(state_store):
    ...
    assert len(buy_signals) >= 1, f"Expected at least 1 BUY signal, got {[s.signal_type for s in all_signals]}"
...
async def test_strategy_bearish_crossover(state_store):
    ...
    assert len(sell_signals) >= 1, f"Expected at least 1 SELL signal, got {[s.signal_type for s in all_signals]}"
...
async def test_strategy_alternate_signals(state_store):
    ...
    assert len(all_signals) >= 4, f"Expected at least 4 signals, got {len(all_signals)}: {signal_types}"
